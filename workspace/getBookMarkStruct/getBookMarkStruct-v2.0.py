...
class getBmStruct():
	...
	# 3)获取书签结构的字典，返回一个字典bmdict,bmdeepdict：字典深度字典{"maxdeep":...,"mindeep":...}
	def getBmStruct(self,htmlstring,bmdeepdict):
		def findnextcontent(htmlstring,bmdeep):		# 只找下一层的文件夹和内容
			bmdict = {
				"dirname":str(),
				"deep":bmdeep,	# 直接返回书签文件夹，及其下属内容
				"sondir":{},
				"directbm":{},
			}
			htmlstringlist = htmlstring.split("\n")
			for line in htmlstringlist:
				if line ==  htmlstringlist[0]:
					bmdict["dirname"] = line.lstrip(" "*4*bmdeep)
				# 先判断是否为又匹配到同等级的文件夹
				if line.startswith(" "*4*(bmdeep+1)) and not line.startswith(" "*4*(bmdeep+2)):
					linecontent = line.lstrip(" "*4*bmdeep)
					if self.tulinkflag not in line:		# 是文件夹
						dirname = linecontent
						bmdict["sondir"][dirname] = bmdeep
					if self.tulinkflag in line:		# 是直接放在其下的书签
						bmUrl = linecontent.split(self.tulinkflag)[1]
						bmTitle = linecontent.split(self.tulinkflag)[1]
						bmdict["directbm"][bmTitle] = bmUrl
			return bmdict
		
		def findalldir(htmlstring,mindeep,maxdeep):				# 匹配所有的文件夹及深度
			alldirdeepdict = dict()			# 返回的字典
			htmlstringlist = htmlstring.split("\n")
			for dirdeep in range(mindeep,maxdeep+1):
				for linecontent in htmlstringlist: # 先遍历内容会有错
					# 判断是否为书签行
					if (self.tulinkflag not in linecontent) and "javascript:" not in linecontent and linecontent.startswith(" "*4*dirdeep) and not linecontent.startswith(" "*4*(dirdeep+1)):
						dirname = linecontent.lstrip(" "*4*dirdeep)
						alldirdeepdict[dirname] = dirdeep
			return alldirdeepdict

		mindeep = bmdeepdict["mindeep"]
		maxdeep = bmdeepdict["maxdeep"]
		bmdict = {}
		alldirdict = findalldir(htmlstring=htmlstring,mindeep=mindeep,maxdeep=maxdeep)	# 获取所有的文件夹
		for dirdeep in range(mindeep,maxdeep+1):
			# 获取该dirdeep下的所有文件夹
			nowdeepdirlist = list()		# 用于存储该dirdeep下的所有文件夹
			for dirname in list(alldirdict.keys()):		
				if alldirdict[dirname] == dirdeep:
					nowdeepdirlist.append(dirname)
			print(dirdeep,"--------->>>",len(nowdeepdirlist))
			
			# 按nowdeepdirlist中的内容对字符串进行分割，放入列表htmlsplitstringlist中
			nowdeepdirstring = str()	# 当前深度文件夹拼接成字符串
			for dirname in nowdeepdirlist:
				nowdeepdirstring += dirname+"\n"
			dirandcontent = str()
			nowdir = str()
			dirno = 0
			for linecontent in list(htmlstring.split("\n")):
				if linecontent.lstrip(" "*4*maxdeep) in nowdeepdirstring:
					if dirno!=0:
						a = findnextcontent(dirandcontent,dirdeep)
						print(a)
						os.system("pause")
					dirandcontent = str()
					dirandcontent += linecontent + "\n"
					nowdir = linecontent.lstrip(" "*4*maxdeep)
					dirno += 1
				if dirno!=0:
					dirandcontent += linecontent + "\n"
			os.system("pause")
