        ...
        while True:
            ...
            if request:
                ...
                if request_filename.isspace() or request_filename==str():                    #如果全为空格或为空字符串
                    with open("D:\Code\html_back_file_dir\index.txt","r") as back_file:
                    ...
                    back_data = str()
                    try:
                        with open("D:\Code\html_back_file_dir\\"+request_filename,"r") as back_file:
                            back_data_head = "HTTP/1.1 200 OK\r\n"
                            back_data_body = back_file.read()
                            back_data = back_data_head+"\r\n"+back_data_body
                    except:
                        ...
                        back_data_body = "<h1>未在数据库找到相应的文件!!!\r\n</h1>"
                        back_data = back_data_head + "\r\n" + back_data_body
                    finally:
                        #数据回传
                        client_socket.send(back_data.encode("utf-8"))
    ...
    def run_forever(self):
        while True:
            ...
            if not client_socket in self.client_socket_list:
                ...
                #调用函数为客户端服务(多线程)
                thread = gevent.spawn(target=self.server2client,args=(client_socket,))
                thread.start()
            if client_socket in self.client_socket_list:
                while client_socket in self.client_socket_list:
                    self.client_socket_list.remove(client_socket)
            print("目前客户端连接个数:", len(self.client_socket_list))

# 用于整个程序的控制
def main():
    #创建WSGI对象
    wsgi = WSGIServer(ip="127.0.0.1")
    #调用run_forever方法
    wsgi.run_forever()
